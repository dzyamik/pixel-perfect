...
class SpritePlaygroundScene extends Phaser.Scene {
    ...
    // @snippet sprite-overlap-checks
    // @title Pixel-perfect overlap with cheap AABB pre-check
    // @desc Always check the AABB first — it's a few subtractions
    // @desc vs scanning every transparent pixel of the sprite. The
    // @desc pixel-perfect call only fires when bounding boxes
    // @desc actually intersect, so a grid of N sprites costs N²
    // @desc cheap checks plus a handful of expensive ones (just
    // @desc the pairs that are visually nearby). Same pattern for
    // @desc sprite-vs-terrain.
    ...
    // @desc between "user uploaded a PNG" and "the new shape
    // @desc collides correctly."
