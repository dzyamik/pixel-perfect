...
class WormsScene extends Phaser.Scene {
    ...
    // @snippet grounded-via-bitmap-probe
    // @title "Is grounded?" via direct bitmap sample
    // @desc Skip the Box2D contact listener for a simple
    // @desc grounded check. Read the player's scene position,
    // @desc convert to bitmap coordinates, and sample a small
    // @desc grid just below the feet. If any pixel is solid,
    // @desc the player can jump. Cheap (a handful of byte
    // @desc lookups) and immune to the per-frame contact-list
    // @desc churn that comes with frequent terrain rebuilds.
